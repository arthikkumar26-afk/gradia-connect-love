...
serve(async (req) => {
  ...
  try {
    ...
    // FALLBACK: Generate questions via AI when no admin questions found
    if (useAiQuestions || (!usingAdminQuestions && questions.length === 0)) {
      // Auto-fallback to AI generation when no manual papers are assigned
      if (!useAiQuestions && !usingAdminQuestions && questions.length === 0) {
        console.log('No manual question papers found for this job. Auto-falling back to AI question generation.');
      }
