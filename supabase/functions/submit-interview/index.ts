  ...
  try {
    ...
    if (shouldProgress) {
      ...
      if (isSlotBookingStage) {
        // For slot booking stages, send slot booking email with /book-slot link
        console.log('Next stage is slot booking, sending slot booking email');
        
        if (resendApiKey) {
          const candidate = interviewCandidate.candidate;
          const job = interviewCandidate.job;
          const companyName = job?.employer?.company_name || job?.organisation || 'Gradia';
          
          if (candidate?.email) {
            const bookSlotLink = `${appDomain}/book-slot?candidateId=${interviewCandidate.id}&stageId=${nextStage.id}&stageName=${encodeURIComponent(nextStage.name)}`;
...
<body style="font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; color: #374151; padding: 20px; background-color: #f9fafb;">
  <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    ...
    <div style="padding: 32px;">
      ...
      <p>Great news! Based on your performance in the Written Test, you have been <strong>automatically advanced</strong> to the next stage.</p>
      <table style="background: #f0fdf4; border: 1px solid #bbf7d0; padding: 20px; border-radius: 8px; margin: 24px 0; width: 100%;">
        ...
        <tr><td style="padding: 8px 0;"><strong>Next Stage:</strong> ${nextStage.name.replace(' Slot Booking', '')} Round</td></tr>
      ...
      <p>Please select a convenient date and time for your ${nextStage.name.replace(' Slot Booking', '')} round by clicking the button below:</p>
      <div style="text-align: center; margin: 32px 0;">
        <a href="${bookSlotLink}" style="display: inline-block; background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; padding: 16px 40px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px; box-shadow: 0 4px 12px rgba(59, 130, 246, 0.4);">
          📅 Book Your Slot Now
        </a>
      </div>
      <div style="background: #fef3c7; border: 1px solid #fcd34d; padding: 16px; border-radius: 8px; margin: 24px 0;">
        <p style="margin: 0; color: #92400e; font-size: 13px;">
          ...
          • Choose a date and time convenient for you<br>
          • Ensure you have a stable internet connection<br>
          • Use a laptop/desktop with camera and screen sharing enabled<br>
          • Please book your slot within 7 days
        </p>
      </div>
      <p>If the button doesn't work, copy and paste this link in your browser:</p>
      <p style="word-break: break-all; color: #3b82f6; font-size: 12px;">${bookSlotLink}</p>
      <p style="margin-top: 32px;">Best of luck! 🍀</p>
      <p style="margin: 0;">The ${companyName} Hiring Team</p>
    </div>
    <div style="background: #f3f4f6; padding: 20px; text-align: center; font-size: 12px; color: #6b7280;">
      ...
      <p style="margin: 8px 0 0 0;"><a href="mailto:support@gradia.co.in" style="color: #3b82f6; text-decoration: none;">Contact Support</a></p>
                  ...
                  `,
                  headers: { 'List-Unsubscribe': '<mailto:unsubscribe@gradia.co.in>' },
                }),
              });
              console.log('Slot booking email sent to candidate:', candidate.email);
            } catch (emailError) {
              console.error('Failed to send slot booking email:', emailError);
            }
          }
        }
      } else if (nextStage.is_ai_automated && newEvent) {
        // For AI-automated test stages, create invitation with interview link
        const expiresAt = new Date();
        expiresAt.setDate(expiresAt.getDate() + 7);
