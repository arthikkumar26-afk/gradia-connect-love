...
interface UnsubscribedEmployer {
  id: string;
  full_name: string;
  email: string;
...
const UnsubscribedEmployers = () => {
  ...
  const [employers, setEmployers] = useState<UnsubscribedEmployer[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { data: allEmployers, error: empError } = await supabase
          .from('profiles')
        ...
        // Get employer IDs that have active subscriptions
        ...
        const subscribedIds = new Set((subs || []).map(s => s.employer_id));
        const unsubscribed = (allEmployers || []).filter(e => !subscribedIds.has(e.id));
        setEmployers(unsubscribed);
      } catch (error) {
  ...
  const filtered = employers.filter(e => {
    ...
    return !q || e.full_name?.toLowerCase().includes(q) || e.email?.toLowerCase().includes(q) || e.company_name?.toLowerCase().includes(q);
  });
      ...
      <div className="min-h-screen flex w-full bg-muted/30">
        ...
        <main className="flex-1 overflow-auto">
          <header className="sticky top-0 z-10 bg-background/95 backdrop-blur border-b border-border px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-xl font-bold">Unsubscribed Employers</h1>
                  <p className="text-sm text-muted-foreground">Employers without active subscriptions</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <div className="relative w-64">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                  <Input placeholder="Search employers..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
                </div>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => exportToExcel(
                    filtered.map(e => ({
                      ...
                      Phone: e.mobile || 'N/A',
                      Joined: e.created_at ? format(new Date(e.created_at), 'MMM dd, yyyy') : 'N/A',
                    })),
                    'unsubscribed_employers'
                  )}
          ...
          <div className="p-6">
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div><p className="text-2xl font-bold">{employers.length}</p><p className="text-sm text-muted-foreground">Unsubscribed Employers</p></div>
                  </div>
                </CardContent>
              </Card>
              <Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Unsubscribed Employers</CardTitle>
                <CardDescription>Employers who do not have an active subscription plan</CardDescription>
              </CardHeader>
              <CardContent>
                {dataLoading ? (
                  <div className="flex items-center justify-center py-8"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>
                ) : filtered.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">No unsubscribed employers found</div>
                ) : (
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {filtered.map((emp) => (
                        <TableRow key={emp.id}>
                          ...
                          <TableCell>{emp.mobile || 'N/A'}</TableCell>
                          <TableCell>{emp.created_at ? format(new Date(emp.created_at), 'MMM dd, yyyy') : 'N/A'}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
...
export default UnsubscribedEmployers;
