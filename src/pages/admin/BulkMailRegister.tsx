      ...
      const { data, error } = await supabase.functions.invoke("sales-chatbot", {
        body: {
          messages: [
            {
              role: "system",
              content: "You are an email copywriter for Gradia EduTech, a recruitment and education platform. Generate professional, engaging email content. Return ONLY the email body text (no subject line). Keep it concise but compelling. Do not include any HTML tags, just plain text with line breaks."
            },
            {
              role: "user",
              content: `Write an email body for: ${aiPrompt}\n\nThe email is from Gradia EduTech platform. Make it professional and include a clear call-to-action encouraging recipients to register on the platform.`
            }
          ]
        },
      });
      if (error) throw error;
      const generatedText = data?.reply || data?.message || "";
      if (generatedText) {
        setEmailBody(generatedText);
        if (!subject) setSubject(aiPrompt.slice(0, 60));
