...
const SignupPortal = () => {
  ...
  const employerPlans = [
    { id: 'basic', name: 'Basic', duration: '1 Month', price: 5000, points: 1000, features: ['Post up to 3 jobs', 'Basic candidate tracking'] },
    { id: 'standard', name: 'Standard', duration: '3 Months', price: 15000, points: 3000, popular: true, features: ['Post up to 10 jobs', 'Candidate tracking', 'Email support'] },
    { id: 'premium', name: 'Premium', duration: '6 Months', price: 29000, points: 5800, features: ['Unlimited jobs', 'Advanced tracking', 'Priority support'] },
  ];
  ...
  const handleSelectPlan = async (planId: string) => {
    ...
    try {
      ...
      const activatePlan = async (paymentMethod: string, paymentRef?: string) => {
        ...
        toast({ title: 'Plan Activated!', description: `${selectedPlan.name} plan activated successfully.` });
      ...
      const { data: orderData, error: orderError } = await supabase.functions.invoke("create-razorpay-order", {
        body: {
          amount: selectedPlan.price,
          currency: "INR",
          receipt: `emp_${user.id.slice(0, 8)}_${planId}`,
        },
      });
      ...
      const options = {
        ...
        handler: async (response: any) => {
          try {
            ...
            await activatePlan('razorpay', response.razorpay_payment_id);
          } catch (err: any) {
        ...
        prefill: { email, name: contactPerson, contact: contactNumber },
        theme: { color: "#10b981" },
        modal: {
              ...
              <Card className="bg-slate-800 border-slate-700 w-full max-w-md">
                <CardContent className="p-8">
                  <div className="text-center mb-6">
                    ...
                    <h2 className="text-2xl font-bold text-white mb-2">Client Dashboard</h2>
                    <p className="text-slate-400">Login to access your employer dashboard</p>
                  </div>
